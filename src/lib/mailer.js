...
const SITE_NAME = env.NEXT_PUBLIC_SITENAME;
...
<div style="border: 1px solid #ccc">
    ...
    <div style="font-size:14px; color:#4a4d63; padding:16px 0 32px;">
        <p style="text-align:center;">
            &copy; <b>${SITE_NAME}</b> - All rights reserved.
        </p>
    </div>
    </div>
</section>
...
export async function sendVerificationEmail(email, token) {
  const url = `${env.NEXT_PUBLIC_BASE_URL}/verify?token=${token}`;
...
export async function sendPasswordResetEmail(email, token) {
  const url = `${env.NEXT_PUBLIC_BASE_URL}/reset-password?token=${token}`;
